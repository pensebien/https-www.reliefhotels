...
2. Keep the integration **outbound-only and non-blocking**: it fires from every real path that confirms or cancels a reservation Relief's own store already accepted — guest Paystack checkout, staff walk-in, Moniepoint, Paystack Terminal, cashier settle, and the manual staff-dashboard action — not just the one dashboard button. It never gates the guest or staff booking-creation path — Relief's internal `getRoomAvailability` check remains the sole authority on whether a room can be booked. (An initial pass only wired the dashboard button; live testing against the sandbox — see `04-build-test-deploy/test-results/integration-test-reports/2026-08-12-rayza-connect.md` — showed that path is rarely hit for real paid bookings, so it was extended to all six confirmation call sites.)
